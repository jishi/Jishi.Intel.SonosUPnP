...
namespace Jishi.Intel.SonosUPnP
{
	public class SonosPlayer
	{
		...
		{
			if (string.IsNullOrEmpty(value)) return TimeSpan.FromSeconds(0);
		...
		public UPnPDevice MediaRenderer
		{
			get
			{
				if (mediaRenderer != null) return mediaRenderer;
		...
		public UPnPService AVTransport
		{
			get
			{
				if (avTransport != null) return avTransport;
				if (MediaRenderer == null) return null;
